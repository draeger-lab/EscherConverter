...
public class Escher2SBGN extends Escher2Standard<Sbgn> {
  ...
  public Sbgn convert(EscherMap escherMap) {
    ...
    org.sbgn.bindings.Map map = builder.createMap(SBGNbuilder.Language.process_description, 0d, 0d, canvas.isSetHeight() ? canvas.getHeight().doubleValue() : getCanvasDefaultHeight(), canvas.isSetWidth() ? canvas.getWidth().doubleValue() : getCanvasDefaultWidth());
    sbgn.setMap(map);
      ...
      for (Map.Entry<String, EscherCompartment> entry : escherMap.compartments()) {
        ...
        if (!(id.equalsIgnoreCase("n") || id.equalsIgnoreCase("e"))) {
          ...
          Glyph compGlyph = builder.createGlyph(id, compartment.getName(),
            SBGNProperties.GlyphType.compartment, compartment.getX() - xOffset,
            compartment.getY() - yOffset, compartment.getWidth(),
  ...
  private void convertNode(Node node, EscherMap escherMap,
    ...
    if (node.isSetType()) {
      switch (node.getType()) {
      case metabolite:
        glyph = convertMetabolite(node, escherMap, multimarkers, xOffset, yOffset);
    ...
    if ((glyph != null) && getInferCompartmentBoundaries() && node.isSetCompartment()) {
  ...
  private Glyph convertExchange(Node node, Map<String, SBGNBase> node2glyph,
    ...
    logger.warning(MessageFormat.format(bundle.getString("Escher2SBGN.cannotConvertExchange"), node.getId()));
    return null;
  }
   ...
   * @return
   */
  private Glyph convertMidmarker(Node midmarker, Map<String, SBGNBase> node2glyph,
    org.sbgn.bindings.Map map, double xOffset, double yOffset) {
    ...
    if (rId != null) {
      if (!node2glyph.containsValue(rId)) {
        ...
        Glyph rGlyph = builder.createGlyph(SBMLtools.toSId(rId), midmarker.isSetName() ? midmarker.getName() : midmarker.getBiggId(), GlyphType.process, convertCoordinate(midmarker.getX(), xOffset) - width/2d, convertCoordinate(midmarker.getY(), yOffset) - height/2d, width, height);
  ...
  private void createTextLabel(TextLabel textLabel, org.sbgn.bindings.Map map,
    ...
    try {
      ...
      Glyph glyph = builder.createGlyph(id, textLabel.getText(), GlyphType.annotation, convertCoordinate(textLabel.getX(), xOffset), convertCoordinate(textLabel.getY(), yOffset), textLabel.isSetWidth() ? SBGNbuilder.toDouble(textLabel.getWidth()) : getLabelWidth(), textLabel.isSetHeight() ? SBGNbuilder.toDouble(textLabel.getHeight()) : getLabelHeight());
      Callout callout = builder.createGlyphCallout(convertCoordinate(textLabel.getX(), xOffset), convertCoordinate(textLabel.getY(), yOffset));
  ...
  private void convertProcess(EscherReaction reaction, EscherMap escherMap,
    ...
    if (midmarker == null) {
      logger.warning(MessageFormat.format(bundle.getString("Escher2SBGN.midmarkerMissing"), reaction));
      return;
    }
    ...
    if (processGlyph == null) {
      ...
      processGlyph = convertMidmarker(midmarker, node2glyph, map, xOffset, yOffset);
    }
    ...
    if (list != null) {
      for (String segmentId : list) {
        ...
        Node node = escherMap.getNode(segment.getFromNodeId().equals(midmarker.getId()) ? segment.getToNodeId() : segment.getFromNodeId());
      ...
      logger.warning(MessageFormat.format(bundle.getString("Escher2SBGN.reactionNodeWithoutSegments"), reaction.getId()));
    }
   ...
   * @return
   */
  private Port createPort(Node node, Glyph reaction, double xOffset, double yOffset) {
    return builder.createPort(createPortId(reaction, node.getId()), convertCoordinate(node.getX(), xOffset), convertCoordinate(node.getY(), yOffset));
  }
   ...
   * @return
   */
  private Arc convertSegments(Metabolite metabolite, EscherReaction reaction, EscherMap escherMap, org.sbgn.bindings.Map map, Map<String, SBGNBase> node2glyph, double xOffset, double yOffset) {
    ...
    Arc arc = builder.createArc(SBMLtools.toSId("r" + reaction.getId() + "_n" + metabolite.getNodeRefId()), sourceBase, targetBase, isProduct ? ArcType.production : ArcType.consumption);
    ...
    if (coeff != 1d) {
      ...
      Glyph cardinalityGlyph = builder.createGlyph(SBMLtools.toSId("cardinality_" + metabolite.getNodeRefId()), StringTools.toString(Locale.ENGLISH, coeff), GlyphType.cardinality);
      double width = getPrimaryNodeWidth() * getReactionNodeRatio();
      double height = getPrimaryNodeHeight() * getReactionNodeRatio();
    ...
    for (int i = 1; i < connectedSegments.size(); i++) {
      ...
      if (nextNode != target) {
        Next next = builder.createArcNext(convertCoordinate(nextNode.getX(), xOffset), convertCoordinate(nextNode.getY(), yOffset));
   ...
   * @return
   */
  private org.sbgn.bindings.Point convertPoint(Point p, double xOffset, double yOffset) {
    return builder.createPoint(convertCoordinate(p.getX(), xOffset), convertCoordinate(p.getY(), yOffset));
  }
   ...
   * @return
   */
  private Glyph convertMetabolite(Node node, EscherMap escherMap, Map<String, Node> multimarkers,
    double xOffset, double yOffset) {
    ...
    Glyph glyph = builder.createGlyph(SBMLtools.toSId(node.getId()), GlyphType.simple_chemical, x, y, height, width, node.isSetBiggId() && (escherMap.getNodes(node.getBiggId()).size() > 1));

    if (node.isSetLabelX() && node.isSetLabelY()) {
      glyph.setLabel(builder.createLabel(node.isSetName() ? node.getName() : node.getBiggId(), convertCoordinate(node.getLabelX(), xOffset), convertCoordinate(node.getLabelY(), yOffset), Double.valueOf(getLabelWidth()), Double.valueOf(getLabelHeight())));
    }
