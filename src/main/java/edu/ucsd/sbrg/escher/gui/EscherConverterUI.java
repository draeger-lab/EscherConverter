...
public class EscherConverterUI extends BaseFrame {
   ...
   */
  private static final transient Logger logger = Logger.getLogger(EscherConverterUI.class.getName());
    ...
    for (int resolution : resolutions) {
      ...
      if (url != null) {
        ...
        if (UIManager.getIcon(key) == null) {
          logger.warning(MessageFormat.format(bundle.getString("EscherConverterUI.couldNotLoadImage"), key));
        }
      } else {
        logger.warning(MessageFormat.format(bundle.getString("EscherConverterUI.invalidURL"), key));
      }
    }
  ...
  protected Component createMainComponent() {
    ...
    tabbedPane.addChangeListener(evt -> {
      GUITools.setEnabled(tabbedPane.getTabCount() > 0, getJMenuBar(), getJToolBar(), BaseAction.FILE_CLOSE, BaseAction.FILE_SAVE_AS);
    });
  ...
  protected File[] openFile(File... files) {
    GeneralFileFilter filter = SBFileFilter.createJSONFileFilter();
    SBPreferences prefs = SBPreferences.getPreferencesFor(GUIOptions.class);

    if ((files == null) || (files.length == 0)) {
      files = GUITools.openFileDialog(this, prefs.get(GUIOptions.OPEN_DIR), false, true, JFileChooser.FILES_AND_DIRECTORIES, filter);
    }
    ...
    if ((files != null) && (files.length > 0)) {
      ...
      if (!notAccepted.isEmpty()) {
        logger.warning(MessageFormat.format("Unable to open {0,choice,1#file|1<the {0,number,integer} files} {1}.", notAccepted.toString()));
      }
    }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if ((evt.getSource() instanceof EscherParserWorker) && evt.getPropertyName().equals(EscherParserWorker.INTERMERIM_RESULTS)) {
      @SuppressWarnings("unchecked")
      List<OpenedFile<EscherMap>> list = (List<OpenedFile<EscherMap>>) evt.getNewValue();
  ...
  private <T> void saveFile(T result, File destination) throws FileNotFoundException {
    SwingWorker<?, ?> writer;
    if (result instanceof Sbgn) {
      writer = new SBGNWritingTask(new OpenedFile<Sbgn>(destination, (Sbgn) result));
    } else if (result instanceof SBMLDocument) {
      writer = new SBMLWritingTask(new OpenedFile<SBMLDocument>(destination, (SBMLDocument) result), this);
    } else {
    ...
    writer.addPropertyChangeListener(evt -> {
      if (evt.getNewValue().equals(SwingWorker.StateValue.DONE)) {
        logger.info(MessageFormat.format(bundle.getString("EscherConverterUI.fileWritten"), destination));
      }
    });
    writer.execute();
  }
  ...
  public File saveFileAs() {
    ...
    JFileChooser fc = GUITools.createJFileChooser(
      dir,
      false, false, JFileChooser.FILES_ONLY, SBFileFilter.createSBGNFileFilter(),
    ...
    if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
      ...
      if ((savedFile != null) && (savedFile.getParentFile() != null)) {
        ...
        if (fc.getFileFilter().getDescription().toUpperCase().contains("SBGN")) {
          ...
          converter = new EscherConverterWorker<SBMLDocument>(map,
              SBMLDocument.class, SBPreferences.getPreferencesFor(
                EscherOptions.class).toProperties());
        }
